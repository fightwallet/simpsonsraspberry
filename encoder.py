...
for filepath in newFiles:
	...
	#first optimization 
    #encodeCommand = 'ffmpeg -i "%s" -vf scale=-2:360 -r 24 -c:v libx264 -profile:v baseline -level 3.0 -preset slow -crf 28 -pix_fmt yuv420p -c:a aac -b:a 64k -ar 44100 "%s"' % (i, o)
	encodeCommand = 'ffmpeg -i "%s" -vf scale=-2:480 -r 24 -c:v libx264 -profile:v baseline -level 3.0 -preset slow -crf 24 -pix_fmt yuv420p -c:a aac -b:a 96k -ar 44100 "%s"' % (i, o)
